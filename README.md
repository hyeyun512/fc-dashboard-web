...
- 시트를 **1,170px 폭**으로 그리고 (표·도넛·항목별 Summary를 화면처럼 좌우로 두어 세로를 줄임)
- 그 안에서 **글씨를 키운 뒤** (표 13 → 15px)
- 화소만 **2배(2,340px)** 로 찍습니다 (DPI를 192로 적어 워드가 제 크기로 넣게 함)

헤드리스 크롬으로 그리므로 이 PC의 화면 배율이나 창 크기와 무관하게 매달 같은 그림이 나옵니다.

| 옵션 | 뜻 |
| --- | --- |
| `--width 1170` | **그리는 폭.** 키우면 칸이 옆으로 펼쳐져 세로가 짧아지고, 글씨는 폭 대비 작아집니다. |
| `--zoom 1` | **통째로 확대하는 배율.** 가로·세로가 같은 비율로 커지고 생김새는 그대로입니다(글씨도 같이 커짐). |
| `--scale 2` | **선명하게 찍는 배율.** 보이는 크기는 그대로 두고 화소만 늘립니다. |

넓게 쓰고 싶으면 `--zoom`이 아니라 `--width`를 키웁니다 — `--zoom`은 세로도 같은 비율로 길어집니다.
