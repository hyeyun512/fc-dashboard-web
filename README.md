...
- 암호가 맞으면 `fc_auth` 쿠키를 심고 **30분** 유지합니다(`AUTH_MAX_AGE`). 로그인한 뒤 30분이 아니라
  **마지막으로 화면을 연 뒤 30분**입니다 — 페이지를 열 때마다 만료 시각을 뒤로 밉니다.
  공용 PC에 자리를 뜬 뒤 링크가 그대로 열리지 않게 하려는 것입니다. 보는 도중에 끊기지는 않습니다 —
  탭 전환·월 선택은 브라우저 안에서 처리되어 서버를 다시 부르지 않으므로, 새로고침하거나 링크를
  다시 열 때만 다시 묻습니다.
...
매달 보고 메일에 붙일 Summary 4장을 이미지로 만듭니다.

```bash
npm run export:images                    # 배포본에서 최신 월
npm run export:images -- --month 6월     # 특정 월
npm run export:images -- --url http://127.0.0.1:3111
```

`보고 이미지/<월>/` 아래에 `1-Humax합계.png` ~ `4-배부액추이.png`와 `붙여넣기용.html`이 생깁니다.
PNG를 하나씩 복사해 붙이거나, `붙여넣기용.html`을 열어 전체 선택(Ctrl+A) → 복사(Ctrl+C) 하면 네 장이
한 번에 들어갑니다. (이 폴더는 `.gitignore` 대상이라 저장소에 올라가지 않습니다.)

### 왜 화면 캡처 대신 이걸 쓰나

화면을 캡처해 붙이면 두 번 손해를 봅니다 — 창 크기대로 찍힌 그림이 메일에서 다시 **늘어나며 뭉개지고**
(1,154px 그림이 1,354px로 들어가던 식), 받는 사람 읽기 창이 좁으면 통째로 **줄어들며 글씨가 작아집니다.**

받는 화면에서 글씨가 얼마나 크게 보이는지는 결국 `글자 크기 ÷ 그리는 폭`으로 정해지므로, 이 스크립트는

- 시트를 **1,300px 폭**으로 그리고 (표·도넛·항목별 Summary를 화면처럼 좌우로 두어 세로를 줄임)
- 그 안에서 **글씨를 키운 뒤** (표 13 → 15px)
- **1.15배로 확대**해 붙이는 크기를 1,495px로 맞추고
- 화소는 **2배(2,990px)** 로 찍습니다 (DPI를 192로 적어 워드가 제 크기로 넣게 함)

헤드리스 크롬으로 그리므로 이 PC의 화면 배율이나 창 크기와 무관하게 매달 같은 그림이 나옵니다.

| 옵션 | 뜻 |
| --- | --- |
| `--width 1300` | **그리는 폭.** 키우면 칸이 옆으로 펼쳐져 **세로가 짧아지고**, 글씨는 폭 대비 작아집니다. |
| `--zoom 1.15` | **통째로 확대하는 배율.** 가로·세로가 **같은 비율로** 커지고 생김새는 그대로입니다(글씨도 같이 커짐). |
| `--scale 2` | **선명하게 찍는 배율.** 보이는 크기는 그대로 두고 화소만 늘립니다. |

붙였을 때 보이는 폭은 `width × zoom`, 파일 화소는 거기에 `× scale`입니다.
**옆으로 더 펼치고 싶으면 `--width`**, **그림만 더 크게 보고 싶으면 `--zoom`**을 키웁니다.
