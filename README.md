...
- 암호가 맞으면 `fc_auth` 쿠키를 심고 **5분** 유지합니다(`AUTH_MAX_AGE`). 공용 PC에 자리를 뜬 뒤
  링크가 그대로 열리지 않게 하려는 것입니다. 보는 도중에 끊기지는 않습니다 — 탭 전환·월 선택은
  브라우저 안에서 처리되어 서버를 다시 부르지 않으므로, 새로고침하거나 링크를 다시 열 때만 다시 묻습니다.
- 쿠키에는 암호가 아니라 해시를 담아, 쿠키를 봐도 암호를 알 수 없고 암호를 모르면 값을 만들 수도 없습니다.
