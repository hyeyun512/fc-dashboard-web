...
/**
 * 로그인 유지 기간 30분 — 로그인한 뒤 30분이 아니라 **마지막으로 화면을 연 뒤 30분**이다.
 * 미들웨어가 통과시킬 때마다 쿠키를 다시 심어 만료 시각을 뒤로 민다(lib과 middleware가 같은 값을 쓴다).
 * 공용 PC에 자리를 뜬 뒤 링크가 그대로 열리는 일이 없도록 하루 종일 열어두지는 못하게 두되,
 * 보고 자리에서 띄워 놓고 쓰는 동안 끊기지는 않을 만큼으로 잡았다 (2026-09-21 5분 → 30분).
 *
 * 탭 전환·월 선택은 브라우저 안에서 처리되어 서버를 부르지 않으므로 만료 시각도 밀리지 않는다.
 * 즉 한 화면을 30분 넘게 열어둔 뒤 새로고침하면 다시 묻는다.
 */
export const AUTH_MAX_AGE = 60 * 30;
